...
contract VotingEscrowHarness is VotingEscrow {
    ...
    constructor(IAMPT amptToken, SmartWalletChecker swc_, string memory name_, string memory symbol_, uint block_, uint ts) VotingEscrow(amptToken, swc_, name_, symbol_) {
        blockNumber = block_;
        blockTimestamp = ts;
